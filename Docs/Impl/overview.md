# Fast AO Baker 要件定義 (Implementation Plan)
本ドキュメントは、Unity上でSubstance Painter級の高速かつ高品質なAOマップのベイクを可能にする拡張機能の機能要件と非機能要件を定義するものです。（曲率マップ機能は現在オミット中）
...
3. 曲率 (Curvature) ベイク機能（将来的な拡張として予約）
離散ラプラス演算子による高速計算: メッシュの曲率をジオメトリから直接算出。
※UI上は現在非表示になっていますが、ロジックは実装済みです。
4. 実行管理とプレビュー
